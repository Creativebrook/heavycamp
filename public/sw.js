const CACHE='heavycamp-shell-v3',SHELL=['/manifest.webmanifest','/icon.svg'];
...
self.addEventListener('push',e=>{let d={};try{d=e.data?e.data.json():{}}catch{}e.waitUntil(self.registration.showNotification(d.title||'HeavyCamp',{body:d.body||'New music is waiting for you.',icon:'/icon.svg',badge:'/icon.svg',tag:d.tag||'heavycamp',data:{url:d.url||'/'}}))});
